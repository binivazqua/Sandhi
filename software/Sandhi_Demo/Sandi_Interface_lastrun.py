...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\52222\\OneDrive\\Desktop\\Sandhi_Demo\\Sandi_Interface_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Indicaciones_5 = visual.TextStim(win=win, name='Indicaciones_5',
        text='¡Terminaste la segunda prueba!\n\nSigue las indicaciones que te aparecerán en pantalla.\n\nPresiona el botón verde para continuar.',
        font='Arial',
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[Boton_Amarillo, Boton_Rojo, Conteo_1, Botones, Cuenta_tarea1],
        )
        ...
        while continueRoutine and routineTimer.getTime() < 1.5:
            ...
            # if Boton_Amarillo is stopping this frame...
            if Boton_Amarillo.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Boton_Amarillo.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
                    Boton_Amarillo.tStop = t  # not accounting for scr refresh
                    Boton_Amarillo.tStopRefresh = tThisFlipGlobal  # on global time
                    Boton_Amarillo.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Boton_Amarillo.stopped')
                    # update status
                    Boton_Amarillo.status = FINISHED
                    Boton_Amarillo.setAutoDraw(False)
            ...
            # if Boton_Rojo is stopping this frame...
            if Boton_Rojo.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Boton_Rojo.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
                    Boton_Rojo.tStop = t  # not accounting for scr refresh
                    Boton_Rojo.tStopRefresh = tThisFlipGlobal  # on global time
                    Boton_Rojo.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Boton_Rojo.stopped')
                    # update status
                    Boton_Rojo.status = FINISHED
                    Boton_Rojo.setAutoDraw(False)
            ...
            # if Conteo_1 is stopping this frame...
            if Conteo_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Conteo_1.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
                    Conteo_1.tStop = t  # not accounting for scr refresh
                    Conteo_1.tStopRefresh = tThisFlipGlobal  # on global time
                    Conteo_1.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Conteo_1.stopped')
                    # update status
                    Conteo_1.status = FINISHED
                    Conteo_1.setAutoDraw(False)
            ...
            # if Botones is stopping this frame...
            if Botones.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Botones.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
                    Botones.tStop = t  # not accounting for scr refresh
                    Botones.tStopRefresh = tThisFlipGlobal  # on global time
                    Botones.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Botones.stopped')
                    # update status
                    Botones.status = FINISHED
                    Botones.setAutoDraw(False)
            ...
            if Cuenta_tarea1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Cuenta_tarea1.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    for thisTrial_2 in trials_2:
        ...
        Trial_2 = data.Routine(
            name='Trial_2',
            components=[Izquierda_2, Derecha, Arriba, Abajo, Instruccion, Cuenta_tarea2],
        )
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # if Izquierda_2 is stopping this frame...
            if Izquierda_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Izquierda_2.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    Izquierda_2.tStop = t  # not accounting for scr refresh
                    Izquierda_2.tStopRefresh = tThisFlipGlobal  # on global time
                    Izquierda_2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Izquierda_2.stopped')
                    # update status
                    Izquierda_2.status = FINISHED
                    Izquierda_2.setAutoDraw(False)
            ...
            # if Cuenta_tarea2 is stopping this frame...
            if Cuenta_tarea2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Cuenta_tarea2.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    Cuenta_tarea2.tStop = t  # not accounting for scr refresh
                    Cuenta_tarea2.tStopRefresh = tThisFlipGlobal  # on global time
                    Cuenta_tarea2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Cuenta_tarea2.stopped')
                    # update status
                    Cuenta_tarea2.status = FINISHED
                    Cuenta_tarea2.setAutoDraw(False)
